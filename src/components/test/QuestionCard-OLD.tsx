...
const QuestionCard = ({ question, onAnswer, onNext, className }: QuestionCardProps) => {
  ...
  const questionText = t('app.current_language') === 'ES' && question?.textEs 
  ? question?.textEs 
  : (question?.text);
  const questionType = question?.tipo || question?.type;
  const questionOptions = t('app.current_language') === 'ES' && question?.optionsEs 
  ? question?.optionsEs 
  : (question?.options);
  ...
  const minLabel = t('app.current_language') === 'ES' && question?.minLabelEs 
  ? question?.minLabelEs
  : (question?.minLabel);
  const maxLabel = t('app.current_language') === 'ES' && question?.maxLabelEs 
  ? question?.maxLabelEs
  : (question?.maxLabel);
    ...
    switch (questionType) {
      ...
      case 'scale':
        if (questionItems && questionItems.length > 0) {
          // Para cada item, usar el valor en scaleAnswers o el valor por defecto 3
          value = questionItems.map((_, index) => String(scaleAnswers[index] || 3));
        } else {
          // Usar el valor en scaleAnswers[0] o el valor por defecto 3
  ...
  const isValid = () => {
    switch (questionType) {
      ...
      case 'scale':
        if (questionItems && questionItems.length > 0) {
          // Para cada item, si no hay un valor en scaleAnswers, asumimos el valor por defecto de 3
          return questionItems.every((_, index) => scaleAnswers[index] !== undefined || true);
        }
        // Si no hay un valor en scaleAnswers[0], asumimos el valor por defecto de 3
        return true; // Siempre válido porque hay un valor por defecto
      default:
        return false;
    }
  };
