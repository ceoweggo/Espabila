...
class DataRepository:
    ...
    @staticmethod
    async def load_traits() -> List[Trait]:
        """Load all personality traits from the database"""
        try:
            traits_data = await test_traits_collection.find({}).to_list(length=None)
            traits = []
    ...
    @staticmethod
    async def load_question_blocks() -> List[QuestionBlock]:
        """Load question blocks from the database"""
        try:
            blocks = []
            
            # Convert cursor to list first to avoid cursor timeout issues
            blocks_data = await test_blocks_collection.find({}).to_list(length=None)
    ...
    @staticmethod
    async def load_celebrities() -> List[CelebrityProfile]:
        """Load celebrity profiles from the database"""
        try:
            celebrities_data = await test_people_collection.find({}).to_list(length=None)
            celebrities = []
    ...
    @staticmethod
    async def load_activities() -> List[Activity]:
        """Load recommended activities from the database"""
        try:
            activities_data = await test_recommended_activities_collection.find({}).to_list(length=None)
            activities = []
    ...
    @staticmethod
    async def load_professions() -> List[Profession]:
        """Load recommended professions from the database"""
        try:
            professions_data = await test_professions_collection.find({}).to_list(length=None)
            professions = []
    ...
    async def load_skills() -> List[Skill]:
        ...
        try:
            skills_data = await test_skills_collection.find({}).to_list(length=None)
            skills = []
    ...
    @staticmethod
    async def load_interests() -> List[Interest]:
        """Load interests from the database"""
        try:
            interests_data = await test_interests_collection.find({}).to_list(length=None)
            interests = []
    ...
    @staticmethod
    async def load_advices() -> List[Advice]:
        """Load advice recommendations from the database"""
        try:
            advices_data = await test_advices_collection.find({}).to_list(length=None)
            advices = []
